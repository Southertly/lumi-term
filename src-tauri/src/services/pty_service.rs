...
fn build_shell_command(shell: &str, _cwd: Option<&Path>) -> (String, Vec<String>) {
    ...
    let args = if is_cmd {
        vec!["/d".to_string(), "/k".to_string(), "chcp 65001 >nul".to_string()]
        ...
        || resolved_shell.to_lowercase().contains("pwsh")
    {
        vec![
            ...
            concat!(
                ...
                "$global:_lumi_first = $true; ",
                "$function:prompt = { ",
                  "if (-not $global:_lumi_first) { ",
                    "$code = if ($global:LASTEXITCODE -ne $null) { $global:LASTEXITCODE } ",
                           "elseif ($?) { 0 } else { 1 }; ",
                    "Write-Host \"`e]133;D;$code`e\\\" -NoNewline ",
                  "}; ",
                  "$global:_lumi_first = $false; ",
